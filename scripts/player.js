...
//
// Solution — Wall-Clock Tracker + Nudge Method:
//   Worker calculates position safely in background.
//   Watchdog nudges playhead to duration on server disconnects
//   (common with Filegarden/Dropbox) to force the native, trusted 
//   'ended' event to fire, bypassing Android Autoplay blocks.
// ══════════════════════════════════════════════════════════════════
...
export const player = {
  ...
  async playSong(song) {
    ...
    stopFade(); 
    // audio.pause() REMOVED HERE to prevent OS from dropping wake lock
...
setInterval(() => {
  ...
  if (!audio.paused && !trackTransition) {
    ...
    // ── THE NUDGE FIX ──
    // If we are within 2.5 seconds of the end and Filegarden/Dropbox drops the connection...
    if (rem <= 2.5 || (!isNaN(audio.duration) && audio.currentTime >= audio.duration - 0.5)) {
      console.log("[Watchdog] Server connection dropped at end of file. Nudging to trigger 'ended' event.");
      try {
