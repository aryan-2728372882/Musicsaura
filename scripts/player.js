...
function _wcTickStart() {
  _wcTickStop();
  wc.timer = setInterval(_wcTick, WC_TICK_MS);
  _wcTick(); // immediate first tick
}

function _wcTickStop() {
  if (wc.timer) { clearInterval(wc.timer); wc.timer = null; }
}
...
function _wcTick() {
  ...
  // Fire when wall clock says song is WC_ADVANCE_S seconds from end.
  // This is the fix for Android screen-off "stuck on same song" bug.
  if (!wc.fired && wc.duration > 0 && remaining <= WC_ADVANCE_S) {
    wc.fired   = true;
    wc.playing = false;
